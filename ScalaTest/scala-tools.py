...
#     print
    ...
    playlist_filter.value        = u'Tobias Playlist'
    ...
    print u'PlaylistItem ID:', uploaded_pl_item[0].id
    
    #create schedule
    #first get channel
    our_channel_criteria = scws.TObj(column=u'name', restriction=u'EQUALS', value=u'3x1 1600x900')
    our_channel = content_manager.ChannelRS.list(searchCriteria = our_channel_criteria)
    print u'Channel ID:', our_channel[0].id
    print u'Channel Name:', our_channel[0].name
    
    #get all frames from channel
    frames = content_manager.ChannelRS.getFrames(channelId=our_channel[0].id)
    print u'Frame ID:', frames[0].id
    print u'Frame name:', frames[0].name
    
    #remove old timeslots for frame
    timeslots = content_manager.ChannelRS.getTimeslots( {u'channelId':our_channel[0].id}, frameId=frames[0].id)
    for timeslot in timeslots:
        content_manager.ChannelRS.deleteTimeslot(timeslotId=timeslot.id)
        print u'Deleted old new_timeslot ID:', timeslot.id
